...
router.post(
  ...
  (req, res) => {
    try {
      readXlsxFile("./uploads/" + req.fileName).then((rows) => {
        for (var i = 1; i < rows.length; i++) {
          ...
          if (parseInt(i) + 1 === rows.length) {
            fs.unlink("./uploads/" + req.fileName, function (err) {
