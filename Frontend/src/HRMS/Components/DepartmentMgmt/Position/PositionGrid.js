...
export const PositionGrid = ({ positions }) => {
  ...
  const deletePosition = async () => {
    try {
      setLoading(true);
      await axios
        .delete(BASE_URL + `dropdown/deleteDropdownValue?id=${position.id}`, {
          headers: {
    ...
    <div className="mt-3">
      ...
      >
        <Box className="muiModal p-4" sx={{ width: 400 }}>
          ...
          >
            <Button variant="contained" color="error" onClick={deletePosition}>
      ...
      <EditPosition open={open} handleClose={handleClose} id={id} />
      <Snackbar
        open={snackbar}
